...
# CONFIG
# =========================
RUN_TAG = "dynamic_joint_generatedK3_mismatch40_smoketest_seed123"
INPUT_PATH = "coco_work/subsets/coco_train_500_generatedK3_mismatch40_seed123.json"
...
def extract_caption_records(record):
    caps = record["generated_captions"]
    texts = [cap["text"] for cap in caps]
    labels = [cap.get("label", "unknown") for cap in caps]
    corrupted = [bool(cap.get("is_corrupted", False)) for cap in caps]
...
for epoch in range(EPOCHS):
    ...
    for idx, record in enumerate(dataset, start=1):
        ...
        captions, labels, corrupted_flags = extract_caption_records(record)
