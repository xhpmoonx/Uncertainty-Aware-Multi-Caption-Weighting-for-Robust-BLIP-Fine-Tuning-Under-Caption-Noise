...
configs = [
    ...
    ("generatedK3_drop40",    "coco_work/subsets/coco_train_500_generatedK3_drop40_seed123.json"),
    ("generatedK3_drop60",    "coco_work/subsets/coco_train_500_generatedK3_drop60_seed123.json"),
...
out_dir = Path("job_scripts_dynamic_joint")
out_dir.mkdir(exist_ok=True)

for tag, input_path in configs:
    script_path = out_dir / f"run_dynamic_joint_{tag}.sh"
    script = f"""#!/bin/bash
#SBATCH --job-name=dj_{tag}
...
#SBATCH --output=./blip_uncertainty/logs/dynamic_joint_{tag}_%j.out

module purge
module load pytorch/nvidia/22.12
cd ./blip_uncertainty

pytorch coco_work/scripts/train_dynamic_joint_phase2_cli.py \\
  --run_tag dynamic_joint_{tag}_seed123 \\
