...
out_dir = Path("job_scripts_uniform")
out_dir.mkdir(exist_ok=True)

for tag, input_path in configs:
    script_path = out_dir / f"run_uniform_{tag}.sh"
    script = f"""#!/bin/bash
#SBATCH --job-name=uf_{tag}
...
#SBATCH --output=./blip_uncertainty/logs/uniform_{tag}_%j.out
...
pytorch coco_work/scripts/train_uniform_cli.py \\
  --run_tag uniform_{tag}_seed123 \\
